...
用例集版本里的成员摘要比对。两份摘要相同**当且仅当有效值相同**（同一份判据）：覆盖成它本来就等于
的值时两份摘要仍然相同（「有覆盖」不蕴含「摘要不同」，反过来也不能用摘要相同推断「没有覆盖过」，
那次到底给了什么由 `last_definition_overrides` 回答）。控制台据此分两支说话（不会把覆盖说成
「定义被改了」），只在摘要确实不同时才补一句「判据与当前定义不一致」。
