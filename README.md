...
「上一批 20 条里 18 条通过」这句话必须说得出前提。因此提交套件的**那一刻**，这一批用到的用例定义
会被固化成一份**可寻址的版本**：套件与每个格子都带着版本标识 `cs1:<摘要>`，按标识可以反查
**当时**那一份定义（`GET /v1/case-set-versions/{id}`）。用例后来被改成什么样，都不改变历史批次的
归属——它跑的就是当时那一份。

标识是**内容决定的**：同一份定义必然得到同一个标识，因此「这两个套件是不是同一版用例」由标识本身
回答，不需要事后比对。计入标识的是**判据相关**的那一面（来源运行、起点与终点、断言集、回放模式 /
模型 / Prompt 正文覆盖）：

* 断言按 `AssertionSpec` 归一化后再计入，且**字典键序无关**：`{"type": "no_error"}` 与显式写满
  null 的同一断言是同一份定义（接口建用例走模型、Agent 自带用例走原始 dict，两种写法都真实存在）；
* **值里的空白是有意义的**：断言期望值 `"X"` 与 `" X "` 不是一回事，不做 trim；
* **列表顺序参与标识**：断言顺序同时决定断言结果列表的顺序，是判据呈现的一部分；
* **用例的提交顺序不参与**：「这批怎么排」由套件自己的 `case_ids` / 格子 `position` 记录；
* 展示用的名字、描述与筛选用的 `labels` **不参与**：改个名字不是换了一份定义；
* 执行记账（`last_status` / `last_results` / `last_run_at` / `last_condition` 等）**不参与**：
  它们每跑一次就变，算进去的话同一份定义跑第二遍就会「换了定义」，版本化立刻变成废话。

**用例变了要能一眼看出来。** 套件详情给出 `case_set.drift`（哪些用例自本批之后被改动、哪些已经
找不到），控制台在批次那一行显示版本与这句提示，并且**同时说清归属没有变**——只说「用例变了」
会让读者以为这一批的结论也跟着变了。用例卡片上还会显示这条用例当前定义的摘要，以及「最近一次结论
是在另一版定义下得出的」这句提示（没有记录过的那种结论不会冒充一致）。

**「最近一次结论用的是哪一版定义」包含这次执行的覆盖。** 用例行上那一份是*定义*；一次执行真正
按着跑的那一份是**有效定义** = 定义 ⊕ 这次执行**显式给出的覆盖**（`from_seq` / `preset` /
`policy` / `model` / `system_prompt`——即 `POST /v1/cases/{id}/run` 请求体里真的写了的那些字段，
批量格子则由当列条件带下来）。**摘要说明「按什么判的」，覆盖说明「与用例定义差在哪」**：
接口上分别是 `last_definition_digest` 与 `last_definition_overrides`。

* 没有覆盖时两份摘要相同，因此默认路径下可以直接与用例集版本里的成员摘要比对；
* 两份摘要相同**当且仅当有效值相同**（同一份判据）。因此「有覆盖」**不蕴含**「摘要不同」：把模型
  覆盖成用例自己的模型时，两份摘要仍然相同；反过来也不能拿「摘要相同」推断「没有覆盖过」——那次
  到底给了什么由覆盖那一列回答。控制台说的是「最近一次结论是在执行覆盖下得出的（从第 1 步开始 /
  模型 X / Prompt 正文 / 回放模式 Y）」，并只在摘要确实不同时才补一句「判据与当前定义不一致」；
  **不会**把它说成「当前定义已改动」，那句话没有记录支撑。

预算不算覆盖：它约束的是这次跑多少，不是判据是什么（原因见 [上报协议](docs/protocol.md) 第 7 节）。

**批次中途被改也不撒谎。** 每格执行的是**提交那一刻冻结的那一份定义**，执行期不再回头读用例行。
因此不存在「前半批按旧断言、后半批按新断言，却报成一个数」：整批共享同一个版本，批次执行期间
怎么编辑都不会漏进来（用例行被删掉也一样，格子照样按冻结的定义给出结论）。

**改动用例**走 `PATCH /v1/cases/{id}`（断言、起点、来源运行、模式 / 模型 / Prompt 覆盖、名字与
标签）。只改给出的字段；已经落库的结论一个字都不动。
...
`GET /v1/suites/{id}` 返回总数、已完成数、按条件分组的汇总、整批的预算记账
（已用 / 上限 / 是否触顶 / 有多少格子因此没跑），以及这一批的用例集版本
（`case_set`：标识、用例数、用例有没有被改过）。单条运行路径 `POST /v1/cases/{id}/run` 行为不变。
...
| [真实失败任务集](docs/pi-failure-suite.md) | 用真实模型跑「baseline 真的会失败」的任务集：两轮结果、翻转与未翻转、失败性质、成本与预算、限制 | 想知道「改完到底有没有变好」这句话现在有多少证据的人 |
...
- 不含 LLM Judge，也不含跨批次的历史趋势看板与成本对比图表；这些是下一阶段的内容。单批内的条件矩阵与
  按条件分组的汇总已可用，且刻意不提供任何跨条件的合成总分。
- **用例集版本（已交付）与数据集版本管理（未做）不是一回事**：当前能回答「这批跑的是哪一版用例」，
  能取回某一版固化的定义，也能看出用例自那之后有没有被改动。**仍未做**的是：跨版本的对比视图
  （把两版用例的结论并排比较）、版本迁移 / 合并 / 分支工具、跨批次的趋势看板，以及用例集的
  命名版本（现在只有内容决定的标识，没有可编辑的版本名与发布记录）。
- **历史批次没有版本记录**：版本化之前创建的套件读出来就是「无版本记录」，不会按当前用例反推一个
  版本号补上去。那批结论的归属无法追溯——这是事实，不是可以靠回填补上的缺口。
