"""自带 mock 的 SRE 工具集（Agents SDK 版）。

返回的数据与 examples/langgraph_sre_agent 完全一致——那是刻意的，两边要能回答同一个
问题。真正的差异在**声明方式**上：

* LangChain 的 @tool 产出 StructuredTool，副作用声明贴在对象上即可被解析；
* Agents SDK 的 @function_tool 产出 FunctionTool（dataclass），声明同样贴在对象上，
  但对象被包装（录制与回放都要包一层）时声明不会跟着 dataclasses.replace 走，
  因此适配层在包装后会重新贴上声明——见 sdk/agent_flight_recorder/openai_agents.py。

AgentSpec.tool_side_effects 的按名字声明两条路都支持，这里两种都给了。
"""
