"""命令行跑一次示例场景并上报到本地平台。
    ...
    python -m sre_agent.run --endpoint http://127.0.0.1:7710
    python -m sre_agent.run --prompt grounded     # 用修正后的 Prompt
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="跑一次示例 SRE 调查场景并录制")
