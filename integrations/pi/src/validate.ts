...
const {values}=parseArgs({options:Object.fromEntries(['repo','provider','model','out','auth-path','models-path','endpoint'].map(k=>[k,{type:'string' as const}]))});
...
async function main() {
  ...
  try {
    ...
    for(const task of suite.tasks) for(const evidence of task.evidence) {
    ...
    for(const task of suite.tasks) {
      ...
      for(const arm of ['baseline','grounded'] as const) for(let sample=1;sample<=3;sample++) {
        const child=fresh(question,prompts[arm],provider,model,suite.commit);child.mode='regress';child.parent=stored.id;
        await run({bundle:child,parent:stored,authPath:values['auth-path'] as string,modelsPath:values['models-path'] as string});
    ...
    const result={commit:suite.commit,provider,model,negativeControl:negative,stopReason,rows};
    ...
    await writeFile(path.join(out,'report.md'),`# pi 代码调查回归验证\n\n固定提交：${suite.commit}。模型：${provider}/${model}。\n\n${summaries.join('\n\n')}\n\n${stopReason??'所有计划样本已执行。'}\n\n| 任务 | Prompt | 样本 | 结论 | 真实模型调用 | 耗时 ms | 原因 |\n| --- | --- | --- | --- | --- | --- | --- |\n${table}\n\n评测要求结构化答案和准确源码引用；失败也可能是格式或引用不匹配，需要结合回放包复核。参数变化导致的录制不足单列，不能解释为模型退化。没有观察到失败到通过的变化时，不声称修复效果。负向控制为人工构造，不计入真实样本。\n`);
